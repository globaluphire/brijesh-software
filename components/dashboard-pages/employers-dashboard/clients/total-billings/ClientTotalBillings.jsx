...
const ClientTotalBillings = ({ fetchedClientData }) => {
    ...
    async function findInvoice(searchInvoiceDateFrom, searchInvoiceDateTo, searchFilters) {
        ...
        let query = supabase
            .from("invoice_view")
            .select("*")
            .eq("client_name", fetchedClientData.client_name);
    ...
    async function fetchedInvoice() {
        ...
        if ( Object.keys(fetchedClientData).length !== 0 ) {
            try {
                                    ...
                                    <Form.Group className="chosen-single form-input chosen-container mb-3">
                                        { fetchedInvoicedataCSV.length > 0 ?
                                            <CSVLink
                                                ...
                                                filename={"Raftaar-" + fetchedClientData.client_name + "-invoice.csv"}
                                            >
                                                Export to CSV
                                            </CSVLink>
                                        : "" }
...
export default ClientTotalBillings;
