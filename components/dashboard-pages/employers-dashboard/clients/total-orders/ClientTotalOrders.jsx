...
const ClientTotalOrders = ({ fetchedClientData }) => {
    ...
    async function fetchedCSVData() {
        if (fetchedClientData && fetchedClientData.client_name) {
            try {
                let { data: csvOrderData, error } = await supabase
                        .from("csv_orders")
                        .select("*")
                        .eq("Client Name", fetchedClientData.client_name);
                                    ...
                                    <Form.Group className="chosen-single form-input chosen-container mb-3">
                                        { fetchedOpenOrderdataCSV.length > 0 ?
                                            <CSVLink
                                                ...
                                                filename={"Raftaar-" + fetchedClientData.client_name + "-Orders.csv"}
                                            >
                                                Export to CSV
                                            </CSVLink>
                                        : "" }
