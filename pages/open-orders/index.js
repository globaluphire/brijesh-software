...
const OpenOrders = () => {
  ...
  // useEffect(() => {
  //   setDate([subDays(new Date(), 30), new Date()]);
  // }, [date]);
  ...
  const [fetchedDropLocationDispatchData, setFetchedDropLocationDispatchData] =
    useState([]);
  ...
  async function fetchOpenOrders() {
    ...
    try {
      let query = supabase
        ...
        .lte(
          "order_created_at",
          convertToSearchFilterDateTimeTo(dateRange ? dateRange[1] : new Date())
        );
      ...
      let { data: orderData, error } = await query.order("order_created_at", {
        ascending: false,
        nullsFirst: false,
      });
      ...
      if (orderData) {
        orderData.forEach(
          (i) => (i.order_created_at = dateTimeFormat(i.order_created_at))
        );
        orderData.forEach(
          (i) => (i.order_updated_at = dateTimeFormat(i.order_updated_at))
        );
        orderData.forEach(
          (i) =>
          ...
          onClick={() => {
            ...
            setSelectedOrderDetails((previousState) => ({
              ...
              editedMaterial: references.filter(
                (i) =>
                  i.ref_nm === "materialType" && i.ref_dspl === rowData.material
              )[0],
              editedPriority: references.filter(
                (i) =>
                  i.ref_nm === "priority" && i.ref_dspl === rowData.priority
              )[0],
              editedSize: references.filter(
                (i) => i.ref_nm === "size" && i.ref_dspl === rowData.size
              )[0],
  ...
  const preparingDataForLrDialog = async (lrNumber, row) => {
    ...
    try {
      ...
      if (lrData) {
        ...
        if (row.client_number) {
          ...
          if (clientData) {
            setFetchedClientData(
              clientData.filter((i) => i.client_number === row.client_number)[0]
            );
          }
        }

        if (lrData && lrData.consignee_client_id) {
          let { data: consigneeClientData, error: er } = await supabase
            .from("client")
            .select("*")
        ...
        let { data: locationData, error: pickupLocationError } = await supabase
          .from("location")
          .select("*")
        ...
        let { data: locatinContactData, error: pickupLocationMarketingError } =
          await supabase
        ...
        if (locatinContactData) {
          setFetchedPickupLocationMarketingData(
            locatinContactData.filter(
              (i) =>
                i.location_contact_id === lrData.pickup_marketing_contact_id
            )[0]
          );
          setFetchedPickupLocationDispatchData(
            locatinContactData.filter(
              (i) => i.location_contact_id === lrData.pickup_dispatch_contact_id
            )[0]
          );
          setFetchedDropLocationMarketingData(
            locatinContactData.filter(
              (i) => i.location_contact_id === lrData.drop_marketing_contact_id
            )[0]
          );
          setFetchedDropLocationDispatchData(
            locatinContactData.filter(
              (i) => i.location_contact_id === lrData.drop_dispatch_contact_id
            )[0]
          );
    ...
    return (
      <>
        <Tag
          className="badge"
          style={{
            backgroundColor: getStatusSeverity(rowData.status).color,
  ...
  const quantityInfoRender = (rowData) => {
    return (
      <>
        <span
          style={{
            boxShadow: determinePriorityColor(rowData.priority).style,
            paddingLeft: "inherit",
          }}
        >
      ...
      <>
        ...
        <span>
          {rowData.eway_verified ? (
            <Tag className="badge" style={{ backgroundColor: "green" }}>
              Verified
            </Tag>
          ) : (
            ""
  ...
  return (
    <div className="grid">
      ...
      <div className="col-12">
        <div className="card">
          <div className="flex align-items-baseline">
            ...
            <small>&nbsp;(Total: {fetchedOpenOrderData.length})</small>
      ...
      <LrDialog
        ...
        fetchedPickupLocationMarketingData={fetchedPickupLocationMarketingData}
